...
%% 测量得到的四元数及归一化
q4_acc=eu2q4(roll_acc,pitch_acc,yaw_acc);
q0_acc=q4_acc(:,1);
q1_acc=q4_acc(:,2);
q2_acc=q4_acc(:,3);
q3_acc=q4_acc(:,4);
qsquare1=sqrt(q0_acc.^2+q1_acc.^2+q2_acc.^2+q3_acc.^2);
q0_acc=q0_acc./qsquare1;
q1_acc=q1_acc./qsquare1;
q2_acc=q2_acc./qsquare1;
q3_acc=q3_acc./qsquare1;
...
for k=2:num
    ...
    if (det(Hk1*Pkk_1*Hk1'+R)~=0)                   %%判断逆是否存在，不存在的话直接用量测值作为估计值
        ...
        q000=xhat1(1,k);q100=xhat1(2,k);q200=xhat1(3,k);q300=xhat1(4,k);        %%归一化
...
roll_EKF = atan2((2.*(q2_out.*q3_out+q0_out.*q1_out)),(1-2.*(q1_out.*q1_out+q2_out.*q2_out))).*180./3.14159265;
theta = 2.*(q1_out.*q3_out-q0_out.*q2_out);
pitch_EKF =  -1*asin(theta).*180./3.14159265;
yaw_EKF = atan2((2.*(q1_out.*q2_out+q0_out.*q3_out)),(1-2.*(q2_out.*q2_out+q3_out.*q3_out))).*180./3.14159265;
