...
klta = 3-numofsta;
% klta=0;
rda = alfa*alfa*(numofsta+klta)-numofsta;

W0 = rda/(rda+numofsta);
Wo = rda/(rda+numofsta)+1+blta-alfa*alfa;
Wi = 1/(2*(rda+numofsta));

xhat1=zeros(numofsta,num);%状态估计
xhat1(:,1)=X1(:,1);

sigma = zeros(numofsta,2*numofsta+1);
gama_kk_1 = zeros(numofsta,2*numofsta+1);

gama_kk_1_z = zeros(numofout,2*numofsta+1);

for k=2:num
    %%%
    sigma(:,1) = xhat1(:,k-1);
    sqrt_Px = real(sqrt((numofsta+rda)*Pk1));
    for i=2:numofsta+1
        sigma(:,i) = sigma(:,1) + sqrt_Px(:,i-1);
    end
    for i=numofsta+2:2*numofsta+1
        sigma(:,i) = sigma(:,1) - sqrt_Px(:,i-numofsta-1);
    end

    %%%
    for i=1:2*numofsta+1
        gama_kk_1(:,i) =  xhat1(:,k-1) + T*fx(sigma(:,i),p_filter(k-1),q_filter(k-1),r_filter(k-1));
    end
    %%%
    xkk_1hat1 = W0*gama_kk_1(:,1);
    for i=2:2*numofsta+1
        xkk_1hat1 = xkk_1hat1 + Wi*gama_kk_1(:,i);
    end
    %%%
    Pkk_1 = Wo*((gama_kk_1(:,1)-xkk_1hat1)*(gama_kk_1(:,1)-xkk_1hat1)');
    for i=2:2*numofsta+1
    ...
    %%%
    sigma(:,1) = xkk_1hat1;
    sqrt_Px = real(sqrt((numofsta+rda)*Pkk_1));
    for i=2:numofsta+1
        sigma(:,i) = sigma(:,1) + sqrt_Px(:,i-1);
    end
    for i=numofsta+2:2*numofsta+1
    ...
    %%%
    Pzk = Wo*((gama_kk_1_z(:,1)-zkk_1hat1)*(gama_kk_1_z(:,1)-zkk_1hat1)');
    for i=2:2*numofsta+1
        Pzk = Pzk + Wi*((gama_kk_1_z(:,i)-zkk_1hat1)*(gama_kk_1_z(:,i)-zkk_1hat1)');
    end    
    Pzk = Pzk + R;
    %%%
    Pxkzk = Wo*((sigma(:,1)-xkk_1hat1)*(gama_kk_1_z(:,1)-zkk_1hat1)');
    for i=2:2*numofsta+1
