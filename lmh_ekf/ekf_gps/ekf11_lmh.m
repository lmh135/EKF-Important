...
% ned_v = data(:,17);
% ned_w = data(:,18);
% gps_heading = data(:,16)./180.*3.1415926;
AXAVE=-0.3003;
AYAVE=-0.0350;
AZAVE=-9.7896;
...
g_filter=sqrt(ax.^2+ay.^2+az.^2);
V=[vx';vy';vz'];
X=V;
% X(:,1)=[ned_x(1);ned_y(1);ned_z(1)];
X(:,1)=[0;0;0];
V(:,1)=[0;0;0];
V1=V;
axkk=zeros(3,1);
...
%     nedxyz(:,i)= nedxyz(:,i-1)+T*[(vx(i)+vx(i-1))/2; (vy(i)-vy(i-1))/2; (vz(i)-vz(i-1))/2];
    ax(i)=nCb(1,1)*ax(i)+nCb(1,2)*ay(i)+nCb(1,3)*az(i);
    ay(i)=nCb(2,1)*ax(i)+nCb(2,2)*ay(i)+nCb(2,3)*az(i);
    az(i)=nCb(3,1)*ax(i)+nCb(3,2)*ay(i)+nCb(3,3)*az(i);
end
    AXAVE=mean(ax);
    AYAVE=mean(ay);
    AZAVE=mean(az);
...
%     axkk=gx(ax(k-1),ax(k),ay(k-1),ay(k),az(k-1),az(k),q0_imu(k-1),q0_imu(k),q1_imu(k-1),q1_imu(k),q2_imu(k-1),q2_imu(k),q3_imu(k-1),q3_imu(k));
%      nCb = q4_2_nCb((q0_imu(k-1)+q0_imu(k))/2,(q1_imu(k-1)+q1_imu(k))/2,(q2_imu(k-1)+q2_imu(k))/2,(q3_imu(k-1)+q3_imu(k))/2);
     V1(:,k)=V1(:,k-1)+[(ax(k-1)+ax(k))/2-AXAVE;(ay(k-1)+ay(k))/2-AYAVE;(az(k-1)+az(k))/2-AZAVE]*T;
%      V1(:,k)=V1(:,k-1)+[axkk(1,1);axkk(2,1);axkk(3,1)]*T;

%          V1(:,k) = nCb*[V1(1,k); V1(2,k); V1(3,k)];
%     X(:,k)=X(:,k-1)+V1(:,k-1)*T+[axkk(1,1);axkk(2,1);axkk(3,1)]*T*T/2;
        X(:,k)=X(:,k-1)+V1(:,k-1)*T+[(ax(k-1)+ax(k))/2-AXAVE;(ay(k-1)+ay(k))/2-AYAVE;(az(k-1)+az(k))/2-AZAVE]*T*T/2;

%    V1(:,k)=V1(:,k-1)+T*[(ax_filter(k-1)+ax_filter(k))/2;(ay_filter(k-1)+ay_filter(k))/2;(az_filter(k-1)+az_filter(k))/2];
%      V1(:,k)=V(:,k-1)+T*fxv(V(:,k-1),ax_filter(k-1),ay_filter(k-1),az_filter(k-1),p_filter(k-1),q_filter(k-1),r_filter(k-1),q0_imu(k-1),q1_imu(k-1),q2_imu(k-1),q3_imu(k-1),g_filter(k-1));
% % %       V(:,k)=V(:,k-1)+T*[ax(k-1);ay(k-1);az(k-1)];
%        nCb = q4_2_nCb(q0_imu(k),q1_imu(k),q2_imu(k),q3_imu(k));
%        X(:,k)=nCb*X(:,k);
%         V(:,k) = nCb*[V1(1,k); V1(2,k); V1(3,k)];
% % %       X(:,k)=X(:,k-1)+T*(V(:,k-1)+V(:,k))/2;
%     X(:,k)=X(:,k-1)+T*(V1(:,k)+V1(:,k-1))/2;
end
%  figure;plot(V1(1,:)','-g');hold on;plot(neduvw(1,:)','-r');
figure;plot(V1(1,:)','-b');hold on;plot(X(1,:)','-g');hold on;plot(ned_x,'-r');
figure;plot(V1(2,:)','-b');hold on;plot(X(2,:)','-g');hold on;plot(ned_y,'-r');
figure;plot(V1(3,:)','-b');hold on;plot(X(3,:)','-g');hold on;plot(ned_z,'-r');
figure;plot(V1(1,:)','-b');hold on;plot(X(1,:)','-g');hold on;
figure;plot(V1(2,:)','-b');hold on;plot(X(2,:)','-g');hold on;
figure;plot(V1(3,:)','-b');hold on;plot(X(3,:)','-g');hold on;

% 
% figure;plot(ned_y,'-r');hold on;plot(X(2,:)','-b');hold on;plot(V1(2,:)','-g');
% 
% figure;plot(ned_z,'-r');hold on;plot(X(3,:)','-b');hold on;plot(V1(3,:)','-g');

% figure;plot(ned_y,'-r');hold on;plot(X(2,:)','-b');
% figure;plot(ned_z,'-r');hold on;plot(X(3,:)','-b');
...
% vz_EKF = xhat1(6,:);
% 
% figure;plot(ned_x,'-r');hold on;plot(ned_x_EKF,'-g');
% figure;plot(ned_y,'-r');hold on;plot(ned_y_EKF,'-g');
% figure;plot(ned_z,'-r');hold on;plot(ned_z_EKF,'-g');
% figure;plot(vx,'-r');hold on;plot(vx_EKF,'-g');
% figure;plot(vy,'-r');hold on;plot(vy_EKF,'-g');
% figure;plot(vz,'-r');hold on;plot(vz_EKF,'-g');
